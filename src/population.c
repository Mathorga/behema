...
bhm_error_code_t p2d_populate(
    ...
    for (bhm_population_size_t i = 0; i < population->size; i++) {
        ...
        bhm_error_code_t error = bhm_crx2d_init(&(population->cortices[i]), width, height, nh_radius);
        if (error != BHM_ERROR_NONE) {
            ...
            for (bhm_population_size_t j = 0; j < i - 1; j++) {
                ...
                bhm_crx2d_destroy(&(population->cortices[j]));
            }
            return error;
        }
...
bhm_error_code_t p2d_rand_populate(
    ...
    for (bhm_population_size_t i = 0; i < population->size; i++) {
        // Randomly init the ith cortex.
        bhm_error_code_t error = bhm_crx2d_rand_init(&(population->cortices[i]), width, height, nh_radius);
        if (error != BHM_ERROR_NONE) {
            // There was an error initializing a cortex, so abort population setup, clean what's been initialized up to now and return the error.
            for (bhm_population_size_t j = 0; j < i - 1; j++) {
                // Destroy the jth cortex.
...
bhm_error_code_t p2d_breed(bhm_population2d_t* population, bhm_cortex2d_t** child) {
    ...
    for (bhm_population_size_t i = 0; i < population->parents_count; i++) {
        ...
        do {
            ...
            population->rand_state = bhm_xorshf32(population->rand_state);
    ...
    bhm_error_code_t error = bhm_crx2d_create(
        child,
        child_width,
        child_height,
        parents[0].nh_radius
    ...
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_pulse_window(*child, parents[winner_parent_index].pulse_window);
    if (error != BHM_ERROR_NONE) return error;

    // Pick fire threshold from a random parent.
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_fire_threshold(*child, parents[winner_parent_index].fire_threshold);
    if (error != BHM_ERROR_NONE) return error;
    ...
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_syngen_chance(*child, parents[winner_parent_index].syngen_chance);
    if (error != BHM_ERROR_NONE) return error;

    // Pick synstrength chance from a random parent.
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_synstr_chance(*child, parents[winner_parent_index].synstr_chance);
    if (error != BHM_ERROR_NONE) return error;
    ...
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_max_syn_count(*child, parents[winner_parent_index].max_tot_strength);
    if (error != BHM_ERROR_NONE) return error;

    // Pick inhexc range from a random parent.
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_inhexc_range(*child, parents[winner_parent_index].inhexc_range);
    if (error != BHM_ERROR_NONE) return error;

    // Pick sample window from a random parent.
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_sample_window(*child, parents[winner_parent_index].sample_window);
    if (error != BHM_ERROR_NONE) return error;

    // Pick pulse mapping from a random parent.
    population->rand_state = bhm_xorshf32(population->rand_state);
    winner_parent_index = population->rand_state % population->parents_count;
    error = bhm_crx2d_set_pulse_mapping(*child, parents[winner_parent_index].pulse_mapping);
    if (error != BHM_ERROR_NONE) return error;
    ...
    for (bhm_cortex_size_t y = 0; y < (*child)->height; y++) {
        for (bhm_cortex_size_t x = 0; x < (*child)->width; x++) {
            (*child)->neurons[BHM_IDX2D(x, y, (*child)->width)].max_syn_count = msc_parent.neurons[BHM_IDX2D(x, y, (*child)->width)].max_syn_count;
            (*child)->neurons[BHM_IDX2D(x, y, (*child)->width)].inhexc_ratio = inhexc_parent.neurons[BHM_IDX2D(x, y, (*child)->width)].inhexc_ratio;
        }
    }
...
bhm_error_code_t p2d_crossover(
    ...
    for (bhm_population_size_t i = 0; i < population->size; i++) {
        ...
        if (mutate) {
            error = bhm_crx2d_mutate(child, population->mut_chance, mutate_shape);
            if (error != BHM_ERROR_NONE) return error;
        }
...
bhm_error_code_t p2d_mutate(
    ...
    for (bhm_population_size_t i = 0; i < population->size; i++) {
        bhm_error_code_t error = bhm_crx2d_mutate(&(population->cortices[i]), population->mut_chance, mutate_shape);
        if (error != BHM_ERROR_NONE) return error;
    }
