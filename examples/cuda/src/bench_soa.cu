...
void setup_cortices(
    ...
    c2d_create_soa(even_cortex, cortex_width, cortex_height, nh_radius);
    c2d_create_soa(odd_cortex, cortex_width, cortex_height, nh_radius);
    c2d_set_evol_step_soa(*even_cortex, 0x01U);
    c2d_set_pulse_mapping_soa(*even_cortex, BHM_PULSE_MAPPING_RPROP);
    c2d_set_max_syn_count_soa(*even_cortex, 24);
    ...
    c2d_touch_from_map_soa(*even_cortex, touchFileName);
    c2d_inhexc_from_map_soa(*even_cortex, inhexcFileName);

    // Copy the first cortex properties to the second one.
    c2d_copy_soa(*odd_cortex, *even_cortex);
}
